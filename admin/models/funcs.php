...
function resultBlock($errors,$successes){
	...
	if(count($errors) > 0)
	{
		...
		{
			$errorSuccess = $errorSuccess.$error."<br>";
		}
		/*echo "<div id='error'>
	...
	if(count($successes) > 0)
	{
		...
		{
			$errorSuccess = $errorSuccess.$success."<br>";
		}
		/*echo "<div id='success'>
...
//Reset password for a defined user
function resetPassword($users) {
	global $mysqli,$db_table_prefix;
	$i = 0;
	$newPassword = generateHash("123456");
	...
	foreach((array)$users as $id){
		file_put_contents("resetPwd.txt", "newPassword => ".$newPassword.", id => ".$id);
...
function fetchAllUsers()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		sign_up_stamp,
		last_sign_in_stamp
		FROM ".$db_table_prefix."users ORDER BY last_sign_in_stamp DESC");
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $department, $signUp, $signIn);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'department' => $department, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn);
	}
	$stmt->close();
	return ($row);
}

//Retrieve information for client users less Accent Staff
function fetchAllUsersLessAccent()
{
	global $mysqli,$db_table_prefix; 
	$stmt = $mysqli->prepare("SELECT 
		id,
		...
		sign_up_stamp,
		last_sign_in_stamp
		FROM ".$db_table_prefix."users WHERE department <> 'Accent' AND title <> 'Consultant' ORDER BY last_sign_in_stamp DESC");
	$stmt->execute();
...
function updateDisplayName($id, $display)
{
	...
	if (!$connect) {
		...
		// Continue with main update even if permission update fails
	} else {
		$idResult = mysqli_query($connect, "SELECT user_id FROM uc_users WHERE id = '$id'");
		if ($idResult) {
			$row = mysqli_fetch_assoc($idResult);
			if ($row) {
				$userId = $row["user_id"];
				file_put_contents("user.txt", "User details: $userId, $display, $id");
				mysqli_query($connect, "UPDATE uc_permissions SET name='$display' WHERE orgId='$userId'");
			}
		}
...
function fetchAllPermissions()
{
	...
	$stmt = $mysqli->prepare("SELECT
		...
		ORDER BY
			CASE
				WHEN orgId LIKE 'func%' THEN 1
				WHEN orgId LIKE 'org%' THEN 2
				WHEN orgId LIKE 'ind%' THEN 3
				ELSE 4
			END,
			name ASC
		");
	$stmt->execute();
	$stmt->bind_result($id, $name, $orgId, $status, $home);
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'name' => $name, 'orgId' => $orgId, 'status' => $status, 'home' => $home);
	}
	$stmt->close();
	return ($row);
}

//Retrieve information for systems modules only 
function fetchModulePermissions()
{
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		status,
		home
		FROM ".$db_table_prefix."permissions WHERE orgId LIKE 'func%'
		ORDER BY name ASC
		");
	$stmt->execute();
	$stmt->bind_result($id, $name, $orgId, $status, $home);
...
function fetchPermissionDetails($id)
{
	...
	$stmt->bind_param("i", $id);
	$stmt->execute();
	$stmt->bind_result($id, $name, $orgId);
	while ($stmt->fetch()){
		$row = array('id' => $id, 'name' => $name, 'orgId' => $orgId);
	}
	$stmt->close();
	return ($row);
}
...
function securePage($uri){
	...
	elseif(!isUserLoggedIn()) 
	{
		...
		elseif ($loggedInUser->user_id == $master_account){
			...
			// Don't redirect to bpa.php if we're already on bpa.php
			if (basename($_SERVER['PHP_SELF']) != 'bpa.php') {
