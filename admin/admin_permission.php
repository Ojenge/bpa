...
if(!empty($_POST)){
	...
	else
	{
		...
		if(!empty($_POST['name']) && $permissionDetails['name'] != $_POST['name']) {
		...
		if(!empty($_POST['name']) && !empty($_POST['removePermission'])){
...
if(substr($permissionDetails['orgId'],0,3) == 'org' || $permissionDetails['orgId'] == 'root')
{
	echo "
	<table class='admin'>
	<tr><td valign='top'>
	...
	<p>
	<label>Name:</label>
	<input type='text' name='name' id='name' value='".$permissionDetails['name']."' />
	</p>
	</p>
...
<p>
<label>&nbsp;</label>
<input type='submit' value='Update' onclick='updatePermInfo()' class='submit' />
</p>
</div>";
