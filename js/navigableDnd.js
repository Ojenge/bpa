...
function(Select, request, Memory, Observable, topic, on, json, List, Grid, Selection, Keyboard, DnD, Hider, declare, arrayUtil, DnDSource, lang, Deferred){
...
scorecardStore2 = Observable(Memory({data: data2}));
	...
	var organizations = new scorecardList({ selectionMode: "single" }, "organizations");
	var perspectives = new scorecardList({ selectionMode: "extended" }, "perspectives");
	var objectives = new scorecardList({ selectionMode: "single" }, "objectives");
	...
	function afterDrop()
	{
		...
		for (i = 1; i <= 3; i++)
		{
			...
			{	
				...
				check[i] = true;
				//alert(newValue + " has been added :-). Check value is: " + check[i]);
				
				formulaArea.value = formulaArea.value + newValue;
				formulaArea.focus();
			}
			else
			{//alert(droppedId[i]);
...
// Create collection frequency dropdown independently of AJAX calls
var stateData = [
...
// Check if the collectionFrequency element exists before creating the dropdown
if (document.getElementById("collectionFrequency")) {
	...
	console.warn("collectionFrequency element not found, dropdown not created");
}
