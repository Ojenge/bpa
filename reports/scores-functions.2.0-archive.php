...
function getObjectives($parentId)
{
	$ids = array();
	$arrayPosition = 0;
	$result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id, name FROM objective WHERE linkedObject = '$parentId' ORDER BY sortColumn");
	while($row = mysqli_fetch_array($result))
	{
		$ids[$arrayPosition]["id"] = $row["id"];
		$ids[$arrayPosition]["name"] = $row["name"];
...
function getMeasures($parentId)
{
	$ids = array();
	$arrayPosition = 0;
	$measureList = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id, name, calendarType FROM measure WHERE linkedObject = '$parentId'");
	$count = mysqli_num_rows($measureList);
	while($row = mysqli_fetch_array($measureList))
	{
		$ids[$arrayPosition]["id"] = $row["id"];
		$ids[$arrayPosition]["name"] = $row["name"];
...
function getInitiativesIndividuals($objId, $globalDate)
{
	...
	$result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id, name, dueDate FROM initiative WHERE projectManager = '$objId'") or file_put_contents("error.txt", "Error = > ".mysqli_error());//The Org Scorecard transends years hence the need to pull all initiatives. LTK 05 June 2022 2048 hrs
	while($row = mysqli_fetch_assoc($result))
	{
		$id = $row["id"];
		$dueDate = $row["dueDate"];
...
function individualScore($objectId, $globalDate) //trying to simplify life LTK 10 March 2021 0215hours; Adding date select functionality. LTK 20Aug2021 1530Hrs
{
	...
	FROM initiative 
	WHERE projectManager = '$objectId' AND archive = 'No' AND dueDate LIKE '$year%' ");
	...
	while($row = mysqli_fetch_assoc($avScore))
	{
        ...
        //Removed AND updatedOn LIKE  '$year%' from above query to allow for updates done after the year has passed. This is rather punitive and not practical. LTK 11.01.23 0912hrs
	...
	$kpiScore = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT DISTINCT measure.id, measure.calendarType 
	FROM measure, tree 
	WHERE measure.owner = '$objectId' AND measure.archive = 'No' 
	OR measure.linkedObject = '$objectId' AND measure.archive = 'No'
	...
	while($kpiRow = mysqli_fetch_array($kpiScore))
	{
		...
		$measure_score_query = "SELECT 3score, date AS lastDate FROM $table WHERE measureId = '$measureId' AND date LIKE '$year%' ORDER BY date DESC LIMIT 1";
	...
	else
	{
		...
		/*if($objectId == "ind8")//Weighting
		{
			$avScore = $avScore*15 + $kpiScoreTotal*85;
			$avScore = $avScore / (15+85);
		}
		else*/ 
		if($avScore == "No Score") $avScore = $kpiScoreTotal;
