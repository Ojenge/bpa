...
//
$upload_path = "images/"; 	// where image will be uploaded, relative to this file
$download_path = "upload/images/";	// same folder as above, but relative to the HTML file
...
if( isset($_FILES[$fieldName]) || isset($_FILES['uploadedfileFlash'])){
  ...
  trace(count($_POST['uploadedfiles'])." ");
	...
	$photoId = $_POST["hiddenIndId"];
	$photoPath = "upload/images/".$_FILES['uploadedfiles']['name'][0];
	file_put_contents("test.txt", $photoPath." for id: ".$photoId);
	@mysqli_query($connect, "UPDATE uc_users SET photo = '$photoPath' WHERE user_id = '$photoId'") or file_put_contents("test2.txt", "Can't save photo");;
